...
<div class="form-group {{ $errors->has('image') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="image" type="text" id="image" value="{{ isset($advertisment->image) ? $advertisment->image : ''}}" >
    {!! $errors->first('image', '<p class="help-block">:message</p>') !!}
</div>
<div class="form-group {{ $errors->has('popImage') ? 'has-error' : ''}}">
    <label for="popImage" class="control-label">{{ 'Popimage' }}</label>
    <input class="form-control" name="popImage" type="text" id="popImage" value="{{ isset($advertisment->popImage) ? $advertisment->popImage : ''}}" >
